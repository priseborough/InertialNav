 ...
 */
void Kalman24::CovariancePrediction(Vector3f deltaAngle, 
                                    Vector3f deltaVelocity,
                                    float dt,
                                    bool onGround)
{

    float windVelSigma = single(dt*0.1); // 0.1 m/s/s
    float dAngBiasSigma = single(dt*0.05/3600*pi/180);
    float dVelBiasSigma = single(dt*0.01/60);
    float magEarthSigma = single(dt*10/60); // 10 mgauss per minute
    float magBodySigma  = single(dt*100/60); // 100 mgauss per minute - allow to adapt more rapidly due to changes in airframe
    ...
    float daxCov = sq(dt*50.0/60*pi/180);
    float dayCov = sq(dt*50.0/60*pi/180);
    float dazCov = sq(dt*50.0/60*pi/180);
    float dvxCov = sq(dt*0.5);
    float dvyCov = sq(dt*0.5);
    float dvzCov = sq(dt*0.5);

    float dvx = deltaVelocity.x;
    float dvy = deltaVelocity.y;
    float dvz = deltaVelocity.z;
    float dax = deltaAngle.x;
    float day = deltaAngle.y;
    float daz = deltaAngle.z;
    ...
    float magN = states[18];
    float magE = states[19];
    float magD = states[20];

    float magX = states[21];
    float magY = states[22];
    float magZ = states[23];
#endif
    ...
    for (uint8_t i=0; i<= 23; i++) {
        predP[i][i] = predP[i][i] + processNoise[i];
    }
	...
	// write result to external array
	P = nextP;
}

void Kalman24::predPzeroRows(uint8_t first, uint8_t last)
{
    for (uint8_t row=first; row<=last; row++) {
        for (uint8_t col=1; col<=24; col++) {
            predP(row, col) = 0;
        }
    }
}

void Kalman24::predPzeroCols(uint8_t first, uint8_t last)
{
